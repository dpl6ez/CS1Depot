...
||| return true if function is even otherwise false
oddb: nat -> bool
oddb n = not (evenb n)
...
{-evenb of (S r) starts by assesing if (S r) is in fact of type
nat. After determining this it assesses the function. It then tests
the first two rules of evenb on r. This fails, so the third rule is
applied. r is slotted into at the rule as n. This results in evenb
being applied to the the double pred of the in input. In this case
this results in evenb being applied to (S O) resulting in false
-}
